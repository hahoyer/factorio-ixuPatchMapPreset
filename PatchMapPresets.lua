...
function PatchMapPresets()
    if settings.startup.ixuMatchMapPreset_ForceNoEnemies.value
        ...
        or settings.startup.ixuMatchMapPreset_ForceResearchQueue.value then
        local mapGenPresets = data.raw["map-gen-presets"].default
        for key, value in pairs(mapGenPresets) do PatchMapPreset(mapGenPresets, key) end
    end

    if settings.startup.ixuMatchMapPreset_MediumPowerPole32.value then
        data.raw["electric-pole"]["big-electric-pole"].maximum_wire_distance = 32
    end
    if settings.startup.ixuMatchMapPreset_LegacyMainMenu.value then
...
function EnsureTableForKey(target, key) if target[key] == nil then target[key] = {} end end
